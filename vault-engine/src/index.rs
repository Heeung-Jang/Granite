...
impl MetadataStore {
    ...
    fn from_connection(
        ...
        let stored_metadata = read_schema_metadata(&connection)?;
        match stored_metadata {
            ...
            Some(_) => create_schema(&connection)?,
            None => {
                create_schema(&connection)?;
                write_schema_metadata(&connection, expected)?;
            }
        }
...
fn create_schema(connection: &Connection) -> MetadataStoreResult<()> {
    connection.execute_batch(
        ...
        CREATE INDEX IF NOT EXISTS idx_links_resolved_target
...
mod tests {
    ...
    #[test]
    fn metadata_store_reports_old_schema_before_creating_new_indexes() {
        let expected = IndexSchemaMetadata::new("sqlite", "metadata-v2", "none", 1);
        let stored = IndexSchemaMetadata {
            schema_version: 1,
            backend_name: "sqlite".to_string(),
            backend_version: "metadata-v1".to_string(),
            tokenizer_config: "none".to_string(),
            generation: 1,
        };
        let connection = Connection::open_in_memory().expect("connection");
        connection
            ...
            .expect("old schema");
        write_schema_metadata(&connection, &stored).expect("old metadata");
