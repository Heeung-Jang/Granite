...
```sh
cargo run --manifest-path vault-engine/Cargo.toml --example abi_layout_manifest > docs/architecture/vault-engine-abi-layout-baseline.json
```
...
constants, state constants, search/panel/depth constants, and known FFI error
code strings. It is generated by Rust code so future refactors can regenerate and
diff it instead of hand-maintaining offsets.
...
| `docs/architecture/vault-engine-abi-layout-baseline.json` | C ABI layout | Generated from `cargo run --manifest-path vault-engine/Cargo.toml --example abi_layout_manifest`. |
...
| `docs/benchmarks/artifacts/vault-engine-architecture-read-api-ui-real-redacted-baseline-2026-05-27.json` | Real-vault Swift UI read probe | Passed; search p95 `53.308ms`, inspector p95 `2.524ms`, file tree p95 `1318.858ms`. |
