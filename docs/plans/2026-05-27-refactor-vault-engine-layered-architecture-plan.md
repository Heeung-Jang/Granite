...
- [x] **RA04.11 Post-adapter performance gate**
  ...
  - Evidence: release fixture artifacts were written under `/tmp/granite-ra0411-fixture.t31BCc`. The comparable fixture `backend-benchmark` rerun with the RA00 baseline inputs reported time-to-usable `153,769us` versus baseline `153,038us`, peak RSS `27,508,736` bytes versus baseline `34,996,224`, Tantivy initial index `152,870us` versus baseline `152,436us`, Tantivy commit `150,842us` versus baseline `151,451us`, `pipeline_config.writer_memory_budget_bytes = 50000000`, `time_to_usable_samples = 3`, backend stage timings present, and `run_metadata.redaction_enabled = true`.
  - Evidence: fixture `materialize-read-index` rerun reported `194.171ms` versus baseline `187.58ms`, peak RSS `28,540,928` bytes, `metadata_batch_size = 9`, `diagnostic_only = true`, and privacy flags remained false for raw note bodies, raw paths, and absolute paths.
  - Evidence: fixture `read-api-benchmark` with the RA00 baseline inputs reported body search p95 `0.060ms` versus baseline `0.070ms`, file-name search p95 `0.459ms` versus baseline `0.645ms`, backlinks p95 `0.130ms` versus baseline `0.172ms`, and aggregate-only privacy fields. A second read API rerun stayed sub-millisecond but showed expected tiny-fixture microsecond noise, so the first comparable run is used as the gate artifact.
  - Evidence: real-vault Swift read UI probe with the RA00 default query and release Rust dylib passed hard ceiling; file tree p95 `1215.822ms` versus baseline `1318.858ms`, search p95 `44.272ms` versus baseline `53.308ms`, inspector p95 `1.921ms` versus baseline `2.524ms`, and main-thread stalls over `50ms` stayed `0`.
  - Evidence: real-vault backend benchmark was run with the same `--skip-sqlite-fts` condition as RA00 and ignored `/tmp` outputs. Stage/time metrics improved: time-to-usable samples `102.70s`, `115.86s`, `126.29s` versus baseline `120.90s`, `159.90s`, `171.50s`; metadata write `43.56s` versus baseline `48.78s`; Tantivy initial index `57.94s` versus baseline `67.82s`; Tantivy add `35.75s` versus baseline `42.33s`; Tantivy commit `18.42s` versus baseline `21.27s`. The ad hoc query set had `3` queries while the RA00 private query file is intentionally unavailable, so real-vault query p95 and `peak_rss_bytes` were recorded for watch only, not accepted as an apples-to-apples regression comparison. Generated private work directories were removed after extracting aggregate JSON to keep local disk headroom.
